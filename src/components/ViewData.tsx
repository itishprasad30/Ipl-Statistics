...
const ViewData = ({ playerDetails }: any) => {
  return (
    <Table mb={20} mt={40} w={1000}>
      ...
      <tbody>
        <Box as="tr" key={playerDetails.Player_Name}>
          ...
          <Td>{playerDetails.Batting_Hand}</Td>
          <Td>{playerDetails.Bowling_Skill}</Td>
          <Td>
            <Link
              href={`/player/individual-strike-rate/${playerDetails.Player_Name}`}
            >
              <a>View Data</a>
            </Link>
          </Td>
